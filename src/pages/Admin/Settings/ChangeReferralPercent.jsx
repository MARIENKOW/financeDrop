...
import { ADDRESS_MATIC_MAX_LENGTH, NFT_PERCENT_PATTERN } from "../../../validateConfig";
import { enqueueSnackbar } from "notistack";
import { SiteContext } from "../../..";
import { StyledNumberField } from "../../../component/general/Form/StyledNumberField";

const ChangeReferralPercent = () => {
   const theme = useTheme();

   const { data, changeReferralPercent } = useContext(SiteContext);

   const { referralPercent } = data;
   ...
   } = useForm({ mode: "onChange", defaultValues: { referralPercent } });

   useEffect(() => {
      reset({ referralPercent });
   }, [referralPercent]);

   const onSubmit = async (data) => {
      try {
         await changeReferralPercent(data);
         enqueueSnackbar(`Referral percent change was a success!`, {
            variant: "success",
         });
      } catch (error) {
         console.log(error);
      ...
      >
         ...
         >
            <Title
               ...
               label={"Change Referral percent"}
            />
            <StyledNumberField
               register={register("referralPercent", {
...
export default observer(ChangeReferralPercent);
